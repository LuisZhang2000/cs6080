...
for (const i in positiveNums) {
	const number = positiveNums[i];
	positiveSum += number;
}

let evenSum = 0;
let evenNums = numbers.filter(num => num % 2 == 0)
for (const i in evenNums) {
	const number = evenNums[i];
	evenSum += number;
}

let sumAbove400 = 0;
let numsAbove400 = numbers.filter(num => num > 400)
for (const i in numsAbove400) {
	const number = numsAbove400[i];
	sumAbove400 += number;
}
